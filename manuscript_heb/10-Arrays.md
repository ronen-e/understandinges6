...
ו-
<span dir="ltr">`copyWithin()`</span>
אינם ברורים כרגע. זה המצב מפני שהמתודות הללו נוצרו במקור על מערכים בינאריים
<span dir="ltr">(typed arrays)</span>
ולאחר מכן התווספו על מערכים רגילים כדי לשמור על עקביות. ואולם, כפי שתלמדו בסעיף הבא, אם תשתמשו במערכים בינאריים בכדי לשנות את הסיביות עבור ערך נומרי, אותן מתודות הופכות לשימושיות מאוד.

## מערכים בינאריים

מערכים בינאריים הינם מערכים שנוצרו למטרה מיוחדת שהיא עבודה עם ערכים נומריים. תחילתם של מערכים בינאריים עם טכנולוגיית
WebGL,
...
`view1`,
שהוא מצג של כל החוצץ המערכי, ואת המשתנה
`view2`,
...
מצגים מאפשרים לנו לקרוא ולכתוב בכל צורה שנרצה ובכל זמן, ללא תלות באופן בו נשמרו הנתונים קודם לכן. כך למשל כתיבת שני ערכים מסוג
int8
...
הקונסטרקטורים למערכים בינאריים מקבלים מספר סוגי ארגומנטים, לכן יש מספר דרכים ליצור מערכים בינאריים. ראשית, ניתן ליצור מערך בינארי חדש על ידי העברת אותם ארגומנטים ש
`DataView`
מקבל
(
  חוצץ מערכי,
...
הדרך השניה ליצור מערך בינארי היא להעביר מספר בודד לקונסטרקטור. המספר מייצג את מספר האלמנטים
(לא מספר הבתים)
שיהיו במערך.
הבנאי ייצור חוצץ מערכי חדש עם מספר הבתים המתאים כדי לייצג את אותם אלמנטים וניתן לקרוא את מספר האלמנטים במערך על ידי שימוש בתכונה
`length`.
לדוגמה:
...
(4
בתים לכל אלמנט).
בשני המקרים נוצר חוצץ מערכי חדש שניתן לגשת אליו במידת הצורך על ידי התכונה
`buffer`.
...
-->
אם לא מועבר ארגומנט לקונסטרקטור מערך בינארי, הקונסטרקטור מתנהג כאילו הועבר הארגומנט
`0`.
זה יוצר מערך בינארי שלא יכול לשמור נתונים מאחר ואפס בתים מוקצים לחוצץ.

הדרך השלישית ליצור מערך בינארי היא להעביר אובייקט בתור ארגומנט בודד לקונסטרקטור. האובייקט יכול להיות אחד מהבאים:
...
* **אובייקט איטרבילי** -
האיטרטור של האובייקט נקרא כדי לשלוף את הפריטים שיועברו למערך הבינארי. הקונסטרקטור יזרוק שגיאה במידה וישנו אלמנט שאינו תקין עבור סוג המצג.
* **מערך** -
...
אל
`ints2`
מאחר ולשני המערכים יש חוצץ מערכי שונה. אותם מספרים מיוצגים בשני המערכים הבינאריים אך ל
`ints2`
...
## הבדלים בין מערכים בינאריים למערכים רגילים

ההבדל החשוב ביותר בין מערכים בינאריים למערכים רגילים הוא שמערכים בינאריים אינם מערכים רגילים. מערכים בינאריים לא יורשים
מהקונסטרקטור
`Array`
והמתודה
...
מאחר והמשתנה
`ints`
הוא מערך בינארי הוא איננו נחשב מופע של
`Array`
ולא ניתן לזהותו כמערך. ההפרדה הזו חשובה מכיוון
שלמרות שמערכים רגילים ומערכים בינאריים דומים יש עדיין הבדלים רבים ביניהם.

### הבדלים בהתנהגות

בעוד שמערכים רגילים יכולים לגדול ולהתכווץ בזמן שעובדים איתם, מערכים בינאריים תמיד נשארים באותו גודל. לא ניתן להגדיר ערך לאינדקס נומרי שאינו קיים במערך בינארי כמו שאפשר במערכים רגילים, מכיוון ומערכים בינאריים מתעלמים מפעולה כזו.
לדוגמה:
...
הערכים במערכים בינאריים עוברים בדיקות לוודא שרק סוגי נתונים תקינים מופיעים במערך. הערך
0
...
בתוך מערך מסוג
`Int16Array`.
מחרוזות אינן נתון תקין בתוך מערך בינארי, ולכן הערך
`0`
...
כל המתודות שמשנות ערכים במערכים בינאריים פועלות תחת אותן מגבלות. לדוגמה, אם הפונקציה שמועברת למתודה
<span dir="ltr">map()</span>
...
`0`.
הודות להתנהגות זו של תיקון שגיאות, מערכים בינאריים לא צריכים לדאוג לגבי שגיאות כאשר משתמשים בערכים לא תקינים בעת פעולות על המערך.

### מתודות חסרות

בעוד שמערכים בינאריים משתמשים בהרבה מתודות כמו של מערכים רגילים, עדיין חסרות להם מספר מתודות של מערך.
המתודות הבאות אינן זמינות על מערכים בינאריים:
...
המתודות ברשימה למעלה יכולות לשנות את גודל המערך. מערכים בינאריים לא מסוגלים לשנות את גודלם, ולכן מתודות אלו אינן מופיעות במערכים בינאריים.
המתודה
<span dir="ltr">concat()</span>
אינה זמינה כיוון שתוצאת חיבור שני מערכים בינאריים
(
  בייחוד אם הם מכילים סוגי נתונים שווים
)
אינה ידועה והדבר נוגד את מטרת השימוש במערכים בינאריים.

### מתודות נוספות

למערכים בינאריים יש שתי מתודות שלא מופיעות על מערכים רגילים:
המתודות
...
<span dir="ltr">set()</span>
מעתיקה מערך בינארי אחד לתוך מערך בינארי שני, בעוד שהמתודה
<span dir="ltr">subarray()</span>.
מעבירה חלק ממערך בינארי קיים למערך בינארי חדש.

המתודה
<span dir="ltr">set()</span>
מקבלת מערך
(רגיל או בינארי)
וארגומנט מיקום פוטנציאלי שממנו יש להכניס את הנתונים. אם לא נעביר דבר נקודת ההתחלה תהיה באינדקס 0. הנתונים מתוך המערך שהועבר כארגומנט מועתקים למערך הבינארי שהוא המטרה ודואג שרק סוגי נתונים תקינים יועתקו.
דוגמה:
...
ומחזיר מערך בינארי חדש. ניתן להשמיט את שני הארגומנטים כדי לייצר עותק מלא של המערך הבינארי.
לדוגמה:
...
מערכים בינאריים, אינם מערכים מבחינה טכנית, כיוון שאינם יורשים מ-
`Array`,
אך הם נראים ומתנהגים בדומה למערכים. מערכים בינאריים מכילים אחד מתוך שמונה סוגי נתונים נומריים ובנויים כשבבסיסים נמצאים אובייקטים מסוג
`ArrayBuffer`
שמייצגים את הסיביות שמרכיבות את המספר, או סדרת המספרים.
מערכים בינאריים מהווים דרך יעילה יותר לביצוע אריתמטיקה ברמת הסיביות כיוון שהערכים לא משתנים מבחינת הפורמט בו הם שמורים, כמו שקורה בערך מספרי רגיל של ג׳אווהסקריפט.
