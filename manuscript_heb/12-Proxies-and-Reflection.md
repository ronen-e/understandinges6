...
ו
ECMAScript 6
פותחו תוך חשיבה על הפשטה של פונקציות . לדוגמה, לפני
ECMAScript 5
סביבת ג'אווה סקריפט הכילה מאפייני אובייקט בלתי ניתנים לגישה או לכתיבה, ומפתחים לא יכלו להגדיר את האפשרויות של גישה וכתיבה למאפיינים שלהם. ECMAScript 5
כללה
<span dir="ltr">`Object.defineProperty()`</span>
מתודה  שמאפשרת למפתחים לעשות מה שמנוע הג'אווה סקריםט יכל לעשות כבר לפני.

ECMAScript 6
נתנה למפתחים גישה רבה יותר ליכולות של מנוע הג'אווה סקריםט שלפני כן כללו רק אובייקטים מובנים. השפה חשפה  את הפעילות הפנימית של האובייקט דרך
*proxies*,
שהיא מעטפת שיכולה לגשת ולשנות פעולות ברמה הנמוכה של מנוע הג'אווה סקריפט. הפרק הזה יתחיל בתיאור הבעיה שפרוקסי בא לפתור ,ואז ידון איך ליצור פרוקסי ולהשתמש בו בצורה יעילה.

## בעיית מערכים

אובייקט המערכים בג'אווה סקריפט מתנהג בדרכים שמפתחים לא יכלו לחקות  באובייקטים שלהם לפני
ECMASCript 6.
אורך המערך
<span dir="ltr">`length`</span>
המאפיין מושפע כאשר עושים השמה של ערך למערך ספיציפי, וניתן לשנות את נתוני המערך על ידי שינוי הערך של
<span dir="ltr">`length`</span>  .
לדוגמה:
...
מערך
`colors`
מתחיל עם שלושה אייטמים. השמת
`"black"`
ל
`colors[3]`
אוטומטית מגדיל את המאפיין
`length`
ל `4`
. מאפיין  השמה של   אינו
`length`
ל
`2`
יסיר את שני האייטמים האחרונים במערך, וישאיר רק את השניים הראשונים. שום דבר ב ECMAScript 5
מאפשר למפתח להשיג את היכולות הללו , אבל פרוקסי משנה את זה.
...
ניתן ליצור פרוקסי שישתמש במיקום של אובייקט אחר
(נקרא
*target*
*מטרה*)
על ידי קריאה ל
...
*virtualizes*
*ידמה*
את המטרה כך שהפרוקסי והמטרה יופיעו באותו אובייקט והפונקצונאליות תהיה באמצעות הפרוקסי.
...
ה
reflection API,
אובייקט מיוצג על ידי
`Reflect` ,
היא אוסף של מתודות שמנגישות את ההתנהגויות הדיפולטיביות של אותה רמה נמוכה שפרוקסי יכול לשכתב אותן. יש מתודת
`Reflect`
עבור כל מלכודת פרוקסי. המתודות האלו בעלות אותו שם, מעבירות את אותם ארגומנטים כמו מלכודת הפרוקסי שלהן
. טבלה 11-1
מסכמת את ההתנהגויות.
...
כל מלכודת עוקפת התנהגות מובנית באובייקט של ג'אווה סקריפט, מאפשרת ליירט ולשנות את התתנהגות.
אם צריך עדיין את ההתנהגות הדיפולטיבית ,ניתן להשתמש במתודה התואמת ל
reflection API.
היחסים בין פרוקסי ל
reflection API
...
`enumerate`
שנועדה לשנות את האופן שבו מתבצעת  הספירה
`for-in`
ו
<span dir="ltr">`Object.keys()`</span>
על האובייקט. אבל המלכודת של
`enumerate`
הוסרה ב
ECMAScript 7
...
`enumerate`
אינה קיימת עוד בשום סביבת JavaScript ולכן אינה מכוסה בפרק זה.

## לייצר Proxy פשוט

כאשר משתמשים ב
`Proxy constructor`
לייצר פרוקסי, מעבירים 2 ארגומנטים: את ה
target-
ואת ה
handler.
ה
*handler*
הוא אובייקט אשר מגדיר מלכודת אחת או יותר . הפרוקסי משתמש בהתנהגיות הדיפולטיביות חוץ מהמלכודות אשר מגדירים לפעולה מסויימת. בשביל לייצר פרוקסי העברה פשוט אפשר להשתמש ב
handler
בלי מלכודות:
</div>
...
נוצר על
`target`.
הפרוקסי עצמו לא מאחסן את המאפיין הזה, הוא פשוט מעביר את המאפיין ל
`target`.
באופן דומה, הערכים של
`proxy.name`
ו
`target.name`
הם אותו הדבר בגלל שהם שניהם מיוחסים ל
`target.name`.
...
נניח שברצונכם ליצור אובייקט שערכי המאפיינים שלו חייבים להיות מספרים. המשמעות היא שיש לאמת כל מאפיין חדש שנוסף לאובייקט, ויש לזרוק שגיאה אם הערך אינו מספר. על מנת להשיג זאת,ניתן להגדיר מלכודת
`set`
...
הוא המתודה המקבילה ל
`set`
באובייקט ההשתקפות, וזו התנהגות ברירת המחדל עבור פעולה זו. המתודה
<span dir="ltr">`Reflect.set()`</span>
מקבלת את אותם ארבע ארגומנטים כמו
`set`
שנמצא במלכודת הפרוקסי, מה שהופך את המתודה לקלה לשימוש בתוך המלכודת. המלכודת אמורה להחזיר
`true`
אם המאפין נקבע ו
`false` אם לא.
(המתודה
<span dir="ltr">`Reflect.set()`</span>
מחזירה את הערך הנכון על סמך האם הפעולה הצליחה.)

כדי לאמת את ערכי המאפיינים, ההית משתמש במלכודת
 `set`
 ובודק את
 `value`
 שמועבר בפנים. הנה דוגמה:
</div>
...
קוד זה מגדיר מלכודת פרוקסי המאמתת את הערך של כל מאפיין חדש שנוסף ל
`target`.
כאשר
`proxy.count = 1`
מורץ, המלכודת
...
הוא
`proxy`.
לא קיים מאפיין כזה שנקרא
`count`
בתוך
`target`,
אז הפרוקסי מאמת את ה
`value`
ע"י העברה לפונקציה
<span dir="ltr">`isNaN()`</span>.
אם התוצאה היא
`NaN`,
אז המאפיין אינו מספר ושגיאה תיזרק. מאחר והקוד מקצה את
`count` ל `1`, הפרוקסי קורא ל
<span dir="ltr">`Reflectset()`</span>
עם אותם ארבע ארגומנטים שהועברו במלכודת בשביל להוסיף מאפיין חדש.

כאשר
`proxy.name`
הוא מוקצה למחרוזת, הפעולה מתבצעת בהצלחה. מאחר ול
`target`
קיים כבר המאפיין
`name` ,
המאפיין הזה מושמט מבדיקת האימות שנקראית ע"י המתודה
<span dir="ltr">`trapTarget.hasOwnProperty()`</span> .
זה מבטיח שמאפיינים שלא נקבעו שהם לא מספרים עדיין נתמכים.

למרות זאת כאשר
`proxy.anotherName`
מוקצה למחרוזת,  תיזרק שגיאה. המאפיין
`anotherName`
לא קיים על המטרה, לכן הערך שלו צריך לעבור אימות. בזמן האימות, תיזרק שגיאה כי
`"proxy"`
הוא לא ערך מספרי.

כאשר המלכודת הפרוקסי
`set`
מאפשרת לך ליירט כאשר נכתבים מאפיינים חדשים, המלכודת פרוקסי
`get`
מאפשרת לך ליירט כאשר מאפיינים נקראים.

## אימות צורת אובייקט באמצעות מלכודת `get`

אחד ההיבטים המעניינים, ולעיתים מבלבלים של   הוא
JavaScript
כאשר קוראים למאפיינים שלא קיימים הוא לא זורק שגיאה. במקום, הערך
`undefined`
...
`target.name`
יזרוק שגיאה מאחר והמאפיין לא קיים. אבל ג'אווה סקריפט משתמשת רק ב
`undefined`
בשביל הערך של המאפיין
`target.name` .
...
מאחר ואימות המאפיין קורה רק בזמן כשמנסים לקרוא את המאפיין, נצטרך להשתמש במלכודת
`get` .
מלכודת
`get`
...
בדוגמה בזו , מלכודת
`get`
מיירטת פעולת קריאת מאפיין. האופרטור
`in`
משמש כדי לקבוע אם המאפיין כבר קיים ב-
`receiver`.
ה
`receiver`
משתמש עם
...
`has`
ועלול להעניק תוצאה שגויה. שגיאה נזרקת אם המאפיין אינו קיים,  אחרת משתמשים בהתנהגות ברירת המחדל.

קוד זה מאפשר מאפיינים חדשים כמו
`proxy.name`
להתווסף, להיות מורשי כתיבה, ולקרוא מהם בלי שום בעיה. השורה האחרונה מכילה שגיאת דפוס:
`proxy.nme`
צריך  להיות
`proxy.name`
במקום. זה יזרוק שגיאה בגלל ש
`nme`
לא קיים כמאפיין.
